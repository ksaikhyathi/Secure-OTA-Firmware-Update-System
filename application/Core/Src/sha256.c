 ...
 * Optimized for embedded systems with minimal RAM usage.
 *
 * CDAC ACTS PG-Diploma in DESD
 *
 * Key Points for Viva:
 * - SHA-256 produces a 256-bit (32-byte) hash
 * - Even a 1-bit change in input produces completely different hash
 * - One-way function: cannot reverse hash to get original data
 * - Collision resistant: ~2^128 operations to find two inputs with same hash
 * - Used instead of CRC because CRC is NOT cryptographically secure
 */
...
/* ─────────────────────────────────────────────
 * SHA-256 Constants (first 32 bits of fractional parts
